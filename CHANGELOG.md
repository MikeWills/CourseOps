...
- **Tests use `httpx2` instead of `httpx`.** Starlette's TestClient warned on
  every run that `httpx` is deprecated and to install `httpx2`, its successor
  from the same author. Dev dependency only; nothing at runtime changes.
...
- **The role page's tab reads "Mankato Marathon | Course Ops", not
  "Course Ops".** Someone working the net has the map, the setup screen and
...
- **An event note is edited behind a pencil, not in a live box.** The
  first cut rendered each note as an input, as the course notes are, and
  on a phone a thumb landing on the list while scrolling was an edit.
  There is no urgency here to justify one-tap correction, so the note is
