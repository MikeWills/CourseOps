"""Regression tests against an organizer-grade course file.

`consumer_export_course.kml` is synthetic, and generated by
`tools/make_course_fixture.py`. It stands in for a real MapMyRun export of a
marathon that used to live here and has been removed: the file was somebody
else's data and not ours to publish, but the *findings* it produced were worth
keeping, so the generator reproduces its defect profile deliberately.

That profile is the whole point, because hand-written fixtures do not have it:

  * identically-named placemarks distinguished only by `<styleUrl>`
  * consecutive duplicate points left where the exporter joined segments
  * straight-line gaps where the route builder used direct/offroad mode
  * point-to-point, not a loop
  * no folders, and no aid stations

Two bugs were originally caught here that hand-written fixtures missed - the
`<styleUrl>` case above, and hint patterns that could not match inside
`start_marker` - and these tests exist so neither returns.

The numbers asserted below match what the real file measured, so they still mean
what they meant. If the generator changes, expect to update them together.
"""
...
FIXTURE = Path(__file__).parent / "fixtures" / "consumer_export_course.kml"
...
def test_end_to_end_import_of_the_file(tmp_path):
    conn = db.connect(tmp_path / "t.sqlite3")
    db.init_schema(conn)
    event_id = db.create_event(conn, "course_file", "2026 the example course Marathon")
...
def test_the_export_has_no_folders():
    """A real-data finding, recorded so a change cannot quietly assume otherwise.

    The the example course MapMyRun export has no <Folder> elements at all: every placemark
