...
- ~~**Non-ham volunteers cannot be tracked.**~~ Built 2026-09-15, issue
  ...
  **Verified on a real iPhone 2026-09-15**: the first scan found OwnTracks
  refusing the config link until *Settings → Remote Control → Allow
  external configuration* is on (now a step on the card, with the app's
  own screens in `/help/phone-tracking`); with it on, the scan configured
  the app and a fix reached the live map within a minute. **Android not
  yet tried** - the setting is documented as identical; the first event
  with an Android medic rehearses it a week out.
